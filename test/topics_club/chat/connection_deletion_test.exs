defmodule TopicsClub.Chat.ConnectionDeletionTest do
  ...
  test "a session start that wins the deletion race is stopped before row deletion" do
    ...
    assert Repo.get(TopicsClub.Chat.ServerConnection, connection.id) == nil
  end
  ...
  test "a normal disconnect tears down the IRC client before deletion" do
    ...
    assert_receive {:irc_server_line, "NICK topics_club"}, 5_000

    registry_key = {connection.user_id, connection.id}

    assert [{client_pid, nil}] = Registry.lookup(TopicsClub.Irc.ClientRegistry, registry_key)
    ...
    assert Registry.lookup(TopicsClub.Irc.ClientRegistry, registry_key) == []
  ...
  test "a stale live session cannot mutate or publish for a marked connection" do
      ...
      {:ircxd,
       {:join,
        %{
          ...
          nick: "topics_club",
          raw_source: "topics_club!user@test"
        }}}
    )
    ...
    assert Repo.get!(TopicsClub.Chat.ServerConnection, connection.id).nickname == "topics_club"
    assert SessionLocator.whereis(connection) == session_pid
