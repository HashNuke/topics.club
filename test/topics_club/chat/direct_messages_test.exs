defmodule TopicsClub.Chat.DirectMessagesTest do
  ...
  alias TopicsClub.Chat.DirectMessageRenamer
  alias TopicsClub.Chat.DirectMessageSender
  alias TopicsClub.Chat.MessageIngestion
  alias TopicsClub.Chat.{DirectMessageBlockIdentity, Message, MessageHistory, Notification}
  ...
  test "an ingestion waiting behind a block transaction is dropped before persistence" do
    ...
    on_exit(fn ->
      ...
      Ecto.Adapters.SQL.Sandbox.unboxed_run(Repo, fn ->
        if persisted_user = Repo.get(TopicsClub.Accounts.User, user.id),
