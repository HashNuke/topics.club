defmodule TopicsClub.Notifications.PreferencesTest do
  ...
  alias TopicsClub.Notifications.Preferences
  alias TopicsClub.Repo

  setup do
    previous_pause =
      Application.get_env(:topics_club_gateway, :pause_notification_preference_broadcast)
  ...
  test "updates and broadcasts server and channel preferences", context do
    ...
    Phoenix.PubSub.subscribe(TopicsClub.PubSub, "user:#{scope.user.id}")
  ...
  test "revision broadcasts remain ordered when an older update is delayed", context do
    ...
    Application.put_env(
      :topics_club_gateway,
      :pause_notification_preference_broadcast,
      {self(), 1}
    )
    ...
    stored = Repo.get!(TopicsClub.Chat.ServerConnection, connection.id)
