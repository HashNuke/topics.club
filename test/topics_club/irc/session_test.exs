defmodule TopicsClub.Irc.SessionTest do
  ...
  alias TopicsClub.Irc.{CommandRegistry, Session, SessionLocator, SessionSupervisor}
  ...
  defp run_session_scenario do
    ...
    assert :ok = IrcTestServer.broadcast(server, "#pipe", "akash", "hello topics_club")

    assert_receive {:buffer_message, %{body: "hello topics_club", nick: "akash"}}, 1_000

    messages = MessageHistory.list_messages(user, membership.id)
    assert Enum.any?(messages, &(&1.body == "hello topics_club" and &1.nick == "akash"))
               ...
               {:ircxd,
                {:irc_error,
                 %{
                   ...
                   params: ["topics_club", "#invite", "Cannot join channel (+i)"]
                 }}},
               state
             )
               ...
               {:ircxd,
                {:irc_error,
                 %{
                   ...
                   params: ["topics_club", "topics_club", "Nickname is already in use"]
                 }}},
               state
             )
  ...
  test "client-info refresh does not make a confirmed channel rejectable as a pending JOIN" do
    ...
    {:ok, intent} = CommandRegistry.resolve("NICK topics_club2", info)
    ...
    assert :ok =
             IrcTestServer.send_line(
               server,
               ":topics_club-test 403 topics_club2 #room :No such channel"
             )
  ...
  test "records IRC notices, actions, topics, MOTD, and numerics in the right buffers" do
    ...
    assert {:noreply, ^state} =
             Session.handle_info(
               {:ircxd,
                {:notice, %{target: "topics_club", nick: "NickServ", body: "identify please"}}},
               state
             )
  ...
  test "records a safe readable fallback for an unrecognized numeric" do
    ...
    message = %Ircxd.Message{
      source: "irc.example",
      command: "799",
      params: ["topics_club", "opaque context", "A future server reply"]
    }
  ...
  test "fails only the unlabeled query matched by standard reply context" do
    ...
    assert Repo.get!(TopicsClub.Chat.Message, alice_invocation.id).metadata["command_status"] ==
             "sent"

    assert Repo.get!(TopicsClub.Chat.Message, bob_invocation.id).metadata["command_status"] ==
             "failed"
  ...
  test "does not complete an unlabeled JOIN from another user's event and rejects its failure" do
    ...
    {:ok, nick_invocation} =
      CommandMessages.record(connection, buffer_id, "NICK topics_club_", %{
    ...
    nick_spec = Ircxd.CommandSpec.classify("NICK", ["topics_club_"], %{isupport: %{}})
    ...
    failed_invocation = TopicsClub.Repo.get!(TopicsClub.Chat.Message, invocation.id)
  ...
  test "does not let an older PART intent shadow a matching JOIN event" do
    ...
    event =
      Ircxd.Client.Event.from_legacy!(
        {:join, %{channel: "#same", nick: "topics_club", source_self?: true}}
      )
  ...
  test "waits for split ISUPPORT lines before flushing queued joins" do
    assert_queued_join_flush(
      "split-isupport",
      [
        ":topics_club-test 005 topics_club PREFIX=(ov)@+ :are supported",
        ":topics_club-test 005 topics_club CHANTYPES=~ CASEMAPPING=ascii :are supported"
      ],
      "~custom",
      "ascii"
    )
  ...
  test "does not trust a partial ISUPPORT line before MOTD end" do
    server =
      start_supervised!(
        {IrcTestServer,
         {self(),
          motd_end?: false,
          isupport_lines: [":topics_club-test 005 topics_club PREFIX=(ov)@+ :are supported"]}}
      )
    ...
    assert :ok =
             IrcTestServer.send_line(
               server,
               ":topics_club-test 376 topics_club :End of /MOTD command"
             )
  ...
  test "rekeys sent JOIN tracking when late ISUPPORT changes casemapping" do
    ...
    assert :ok =
             IrcTestServer.send_line(
               server,
               ":topics_club-test 005 topics_club CHANTYPES=# CASEMAPPING=rfc1459 :are supported"
             )
  ...
  test "routes negotiated status-message targets through their underlying channel" do
    server =
      start_supervised!(
        {IrcTestServer,
         {self(),
          isupport_lines: [
            ":topics_club-test 005 topics_club CHANTYPES=# STATUSMSG=@+ CASEMAPPING=ascii :are supported"
          ]}}
      )
    ...
    assert Enum.any?(messages, &(&1.body == "hello voiced users" and &1.nick == "topics_club"))
  ...
  test "an envelope JOIN failure rejects only its correlated pending invocation" do
    Enum.each([:unlabeled_461, :labeled_461, :contextless_fail], fn scenario ->
      ...
      assert Repo.get!(TopicsClub.Chat.Message, failed_id).metadata["command_status"] == "failed"
      assert Repo.get!(TopicsClub.Chat.Message, pending_id).metadata["command_status"] == "sent"
