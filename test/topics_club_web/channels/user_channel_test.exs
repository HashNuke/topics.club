defmodule TopicsClubWeb.UserChannelTest do
  use TopicsClubWeb.ChannelCase
  ...
  alias TopicsClub.Chat.MembershipLookup
  ...
  test "disconnects a live channel when its session expires" do
    ...
    TopicsClubWeb.Endpoint.subscribe(session_socket_id)

    expired_at = DateTime.utc_now(:second) |> DateTime.add(-15, :day)

    UserToken
    |> where([token], token.token == ^session_token)
    |> TopicsClub.Repo.update_all(set: [inserted_at: expired_at])
