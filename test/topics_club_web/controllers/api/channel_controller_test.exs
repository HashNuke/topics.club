defmodule TopicsClubWeb.Api.ChannelControllerTest do
  ...
  test "leaves a channel membership through the API", %{conn: conn, user: user} do
    ...
    assert :ok = TopicsClub.Irc.Session.quit(connection)
  end

  test "does not leave another user's channel membership", %{conn: conn} do
    other_user = TopicsClub.AccountsFixtures.user_fixture()
