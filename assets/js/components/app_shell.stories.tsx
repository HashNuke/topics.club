...
export default {title: "App/AppShell", component: AppShell, parameters: {layout: "fullscreen"}, args: {activeChannel: channel, activeServer: server, channelDirectory: {channels: topics.map((topic) => ({channel: topic.channel, topic: topic.description, users: topic.members})), status: "ready"}, commandCatalog: [], composerError: null, connectionHealth: "connected", connections: [server], currentUser: {email: "mira@example.com"}, discoverServerChannels: topics.map((topic, index) => ({id: index + 1, name: topic.channel, topic: topic.description, user_count: topic.members, network_id: 1, network_name: "Example IRC", server_host: topic.server_host, server_port: 6697, use_tls: true})), discoverLoading: false, draft: "", messages, messagesLoading: false, notificationDeviceState: {capability: "granted", configured: true, loading: false, subscribed: true}, notificationSavingIds: new Set(), serverMessages: messages, topics, users, view: "chat", onDiscover: noop, onDisconnectServer: noop, onJoinDirectoryChannel: noop, onJoinDiscoverServerChannel: noop, onJoinThisServerChannel: noop, onJoinManualServer: noop, onLeaveChannel: noop, onLeaveServer: noop, onLoadOlderMessages: noop, onMarkChannelRead: noop, onOpenChannelDirectory: noop, onReadingStateChange: noop, onReconnectServer: noop, onRetryMessage: noop, onRetryRealtime: noop, onSelectChannel: noop, onSelectServer: noop, onSendMessage: (event: React.FormEvent<HTMLFormElement>) => event.preventDefault(), onShowChat: noop, onToggleChannelNotifications: noop, onToggleServerNotifications: noop, onUpdateDraft: noop, onUpdateServer: noop, onSelectTopic: noop}}
export const Chat = {}
