...
export default function ChannelDirectoryControls({manualChannel, onJoinManualChannel, onSearch, onUpdateManualChannel, onUpdateQuery, query}: ChannelDirectoryControlsProps) {
  return (
    <div className="grid gap-3 py-5 md:grid-cols-[minmax(0,1fr)_minmax(18rem,0.55fr)]">
      <form id="channel-directory-search-form" onSubmit={onSearch}>
        <label className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500" htmlFor="channel-directory-search">Search this server</label>
        <div className="flex h-11 overflow-hidden rounded-md border border-slate-700 bg-[var(--app-panel)] transition focus-within:border-cyan-300/70 focus-within:ring-2 focus-within:ring-cyan-300/10">
          ...
          <button className="border-l border-slate-700 px-4 text-sm font-semibold text-cyan-200 transition hover:bg-cyan-300 hover:text-cyan-950" type="submit">Search</button>
        </div>
      </form>
