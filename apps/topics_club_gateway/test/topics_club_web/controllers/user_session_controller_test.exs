defmodule TopicsClubWeb.UserSessionControllerTest do
  use TopicsClubWeb.ConnCase, async: true

  import TopicsClub.AccountsFixtures
  alias TopicsClub.Accounts
  alias TopicsClub.Accounts.{Scope, UserToken}
  alias TopicsClub.Notifications.{PushSubscription, PushRegistrations}
  ...
  describe "POST /users/log-in - email and password" do
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_topics_club_gateway_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
    ...
    test "atomically replaces a same-user session and rebinds its push installation", %{
      ...
      TopicsClubWeb.Endpoint.subscribe(previous_socket_id)
      TopicsClubWeb.Endpoint.subscribe(other_socket_id)
  ...
  describe "POST /users/log-in - magic link" do
    test "sends magic link email when user exists", %{conn: conn, user: user} do
      ...
      assert TopicsClub.Repo.get_by!(Accounts.UserToken, user_id: user.id).context == "login"
    end
