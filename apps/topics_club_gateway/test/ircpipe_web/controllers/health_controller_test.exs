defmodule IrcpipeWeb.HealthControllerTest do
  ...
  test "production SSL policy permits a plain HTTP platform health check" do
    ...
    browser_conn =
      :get
      |> build_conn("/")
      |> Map.put(:host, "ircpipe.example.com")
      |> Plug.SSL.call(Plug.SSL.init(production_ssl_options()))

    assert browser_conn.halted
    assert browser_conn.status == 301
    assert get_resp_header(browser_conn, "location") == ["https://ircpipe.example.com/"]
  end
  ...
  defp production_ssl_options do
    ...
    |> get_in([:topics_club_gateway, IrcpipeWeb.Endpoint, :force_ssl])
  end
end
