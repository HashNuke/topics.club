defmodule TopicsClub.Chat.ConnectionEndpointTest do
  use TopicsClubWeb.DataCase, async: true

  alias TopicsClub.AccountsFixtures
  alias TopicsClub.Chat.ConnectionEndpoint
  ...
  test "retains connection credentials while encrypting them at rest" do
    ...
    assert TopicsClub.Vault.decrypt!(encrypted_server_password) == "network-secret"
    assert TopicsClub.Vault.decrypt!(encrypted_sasl_password) == "account-secret"
  end
