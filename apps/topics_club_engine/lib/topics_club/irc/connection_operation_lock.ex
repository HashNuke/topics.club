defmodule TopicsClub.Irc.ConnectionOperationLock do
  @moduledoc false
    ...
    case Map.get(state.locks, key) do
      nil ->
        ...
        {:reply, :ok,
         state
         |> put_lock(key, lock)
         |> put_monitor(owner_ref, {key, :owner, owner})}
      ...
      lock ->
        ...
        {:noreply,
         state
         |> put_lock(key, lock)
         |> put_monitor(waiter_ref, {key, :waiter, owner})}
    end
  end
  ...
  defp handoff(state, key, waiters) do
    case :queue.out(waiters) do
      {{:value, waiter}, remaining} ->
        ...
        state
        |> put_lock(key, lock)
        |> put_monitor(waiter.ref, {key, :owner, waiter.pid})
