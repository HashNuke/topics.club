defmodule TopicsClub.Chat.ConnectionDeletion do
  @moduledoc false
          ...
          case result do
            ...
            {deleted, event_batch, event_job} ->
              ...
              if ConnectionDeletionEventsWorker.dispatch(event_batch.id) == :ok do
                :ok = Oban.cancel_job(TopicsClub.EngineOban, event_job)
              end
  ...
  defp maybe_pause_delete_after_quiesce(connection) do
    case Application.get_env(:topics_club_engine, :connection_delete_after_quiesce_barrier) do
  ...
  defp maybe_raise_before_delete_mark(connection) do
    case Application.get_env(:topics_club_engine, :connection_before_delete_mark_exception) do
  ...
  defp maybe_pause_delete_after_mark(connection) do
    case Application.get_env(:topics_club_engine, :connection_delete_after_mark_barrier) do
      {test_pid, barrier_ref} when is_pid(test_pid) ->
        test_ref = Process.monitor(test_pid)
  ...
  defp maybe_fail_final_delete(connection) do
    ...
    case Application.get_env(:topics_club_engine, :connection_final_delete_failure) do
  ...
  defp maybe_raise_final_delete(connection) do
    case Application.get_env(:topics_club_engine, :connection_final_delete_exception) do
      {test_pid, exception_ref} when is_pid(test_pid) ->
  ...
  defp maybe_pause_delete_after_commit(event_batch) do
    case Application.get_env(:topics_club_engine, :connection_delete_after_commit_barrier) do
      {test_pid, barrier_ref} when is_pid(test_pid) ->
        test_ref = Process.monitor(test_pid)
  ...
  defp maybe_pause_delete_after_lock(connection) do
    case Application.get_env(:topics_club_engine, :connection_delete_after_lock_barrier) do
      {test_pid, barrier_ref} when is_pid(test_pid) ->
        test_ref = Process.monitor(test_pid)
