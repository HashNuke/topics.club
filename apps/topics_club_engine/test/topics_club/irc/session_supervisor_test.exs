defmodule TopicsClub.Irc.SessionSupervisorTest do
  ...
  test "stopping an already-closed IRC client succeeds" do
    ...
    pid = start_supervised!({TopicsClub.ClosedIrcSession, connection})

    assert {:via, Registry, {TopicsClub.Irc.SessionRegistry, registry_key}} =
             SessionLocator.via(connection)

    assert Registry.lookup(TopicsClub.Irc.SessionRegistry, registry_key) == [{pid, nil}]
    ref = Process.monitor(pid)

    assert :ok = SessionSupervisor.stop_session(connection)
    assert_receive {:DOWN, ^ref, :process, ^pid, :normal}
    assert Registry.lookup(TopicsClub.Irc.SessionRegistry, registry_key) == []
  end
  ...
  test "an abnormal session exit is not reported as a successful stop" do
    ...
    pid = start_supervised!({TopicsClub.CrashingIrcSession, connection})
    ref = Process.monitor(pid)
  ...
  test "stops a transient replacement that starts after session name lookup" do
    ...
    {:ok, original_pid} =
      DynamicSupervisor.start_child(
        SessionSupervisor,
        {TopicsClub.RestartingIrcSession, {connection, test_pid, start_counter}}
      )
  ...
  test "independent session failures do not exhaust the shared supervisor" do
    ...
    Enum.each(1..4, fn _attempt ->
      ...
      {:ok, original_pid} =
        DynamicSupervisor.start_child(
          SessionSupervisor,
          {TopicsClub.RestartingIrcSession, {connection, self(), start_counter}}
        )
