defmodule Mix.Tasks.TopicsClub.CheckBoundaries do
  use Mix.Task
  ...
  def run(args) do
    ...
    if positional != [] or invalid != [] do
      Mix.raise("usage: mix topics_club.check_boundaries [--baseline-ref GIT_REF]")
    end

    Mix.Task.run("compile")

    manifest = Mix.TopicsClub.Boundaries.load!("config/boundaries.exs")
    child_paths = child_project_paths()
    Enum.each(child_paths, &compile_child!/1)
    graph = xref_graph(child_paths)

    with {:ok, summary} <- Mix.TopicsClub.Boundaries.check(manifest, graph),
  ...
  defp check_baseline(ref, manifest) do
    case System.cmd("git", ["show", "#{ref}:config/boundaries.exs"], stderr_to_stdout: true) do
      {source, 0} ->
        ref
        |> then(&Mix.TopicsClub.Boundaries.load_source!(source, "#{&1}:config/boundaries.exs"))
        |> Mix.TopicsClub.Boundaries.check_transition_regression(manifest)
  ...
  defp xref_graph(child_paths) do
    ...
    child_graphs =
      Enum.map(child_paths, fn child_path ->
        ...
        |> Mix.TopicsClub.Boundaries.prefix_graph(child_path)
      end)

    manifest_graph =
      child_paths
      |> project_manifests()
      |> Mix.TopicsClub.Boundaries.manifest_graph()

    Mix.TopicsClub.Boundaries.merge_graphs(root_graphs ++ [manifest_graph | child_graphs])
  end
  ...
  defp xref_graph_for_current_project do
    ...
    try do
      ...
      output_path
      |> File.read!()
      |> Mix.TopicsClub.Boundaries.parse_dot()
    after
      File.rm(output_path)
    end
  end
