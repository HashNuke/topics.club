...
- Deploy exactly one IRC engine service. Combined mode uses one BEAM node; the supported split topology uses one gateway node and one engine node. `TopicsClub.Engine.Marker` provides discovery and rejects a duplicate marker only when the nodes are already connected; it is not partition-safe ownership fencing. Deployment automation must enforce the single-engine topology. Do not add DNS clustering or horizontally scale the engine without first implementing database-backed ownership leases and fencing.
